...
def octantAnalysis(m,mod=5000):
	...
	try: # opening the input excel sheet
		os.chdir(r'C:\Users\Aman Jha\Documents\GitHub\octantAnalysis\input') # making the parent directory as input directory
	...
	try: # opening the input excel sheet
		os.chdir(r'C:\Users\Aman Jha\Documents\GitHub\octantAnalysis\output') # making the parent directory as input directory
		output=op.Workbook()
...
input_path=st.text_input("Upload the path of the input files",placeholder=r"Please enter the path, for example:- 'C:\Users\Aman Jha\Documents\GitHub\octantAnalysis\input\1.0.xlsx' (without inverted commas)")
...
# location=st.text_input("If you want to download the files in a specific path then please state, otherwise it will get saved in output_files folder in project 2:",placeholder=r"Please enter the path, for example:- 'C:\Users\Aman Jha\Documents\GitHub\octantAnalysis\input' (without inverted commas)",value="")

if work:
	# print("aman")
	try:
		os.mkdir(r'C:\Users\Aman Jha\Documents\GitHub\2001EE22_2022\proj2\output')
	except:
		pass
	
	octantAnalysis(input_files,mod)
	
	try:
		if input_path[-5:]==".xlsx":
			dirname, fname = os.path.split(input_path)
			octantAnalysis(fname,mod)
	except:
		if input_path!="":
